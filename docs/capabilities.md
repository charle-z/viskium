...
falsas, procesos inyectados y bases SQLite temporales. La cámara física es opt-in y no forma parte
de la evidencia pública automatizada; por ello no se publica un perfil de dispositivo ni una
...
| Backend OpenCV aislado en proceso | prototyped | `OpenCVProcessCameraBackend` aplica ownership, selección de API, deadlines y cleanup; dobles, fallos inyectados y pruebas del worker cubren la frontera sin hardware | Unplug, busy, sleep/resume y driver hang con hardware identificado |
...
| Snapshot de cámara one-shot | prototyped | El slice de software recorre grant → admission → MCP in-memory → open/warmup/read → PNG en RAM → close con proveedores y workers controlados | Repetir en perfiles identificados y medir CPU/RSS/latencia |
...
| Transporte MCP local | prototyped | Cinco tools versionadas; tests in-memory cubren PNG, receipt, challenge, grant/admission y el launcher stdio tiene gates sin hardware | Cliente stdio externo con cámara, revocación concurrente y soak |
| Challenge visual sintético MCP | validated | Imagen PNG efímera de 512×384, receipt con hash/dimensiones, store RAM TTL/cap, verificación de un intento y prueba de claims antes del proof card | Ejecutar la matriz externa por modelo/host de OpenCode; no implica calidad visual ni renderizado de UI |
